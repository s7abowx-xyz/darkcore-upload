...
export default async function handler(request) {
  ...
  try {
    ...
    if (!url) return json({ status: false, message: 'فشل الرفع إلى Uguu', raw: data }, 500);

    const origin = new URL(request.url).origin;
    return json({ status: true, creator: 'NovaCore', server: 'Uguu', url: origin + '/api/file?u=' + b64url(url) });
